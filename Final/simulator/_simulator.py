...
class ActuatorPosition(ActuatorMotor):
    ...
    def __init__(
        self, kp: float = 1, kd: float = 0, position_range: List[float] = [-100, 100]
    ) -> None:
...
class ActuatorVelocity(ActuatorMotor):
    ...
    def __init__(
        self, kv: float = 1, velocity_range: List[float] = [-100, 100]
    ) -> None:
...
class Simulator:
    ...
    def _init_default_actuators(self) -> None:
        ...
        for name in self.joint_names:
            self.actuator_configs[name] = ActuatorMotor(
                torque_range=default_ranges[name]
            )
    ...
    def update_actuator(
        self, actuator_id: Union[str, int], actuator: ActuatorMotor
    ) -> None:
    ...
    def run(self, time_limit: Optional[float] = None) -> None:
        ...
        try:
            ...
            while not viewer or viewer.is_running():
                ...
                if self.enable_task_space:
                    tau = self.controller(
                        q=state["q"], dq=state["dq"], desired=state["desired"], t=t
                    )
                else:
                    tau = self.controller(q=state["q"], dq=state["dq"], t=t)
                ...
                if t > real_time:
                    ...
                    print(
                        f"Warning: Simulation running slower than real-time by {real_time - t:.3f}s"
                    )
    ...
    def set_joint_damping(self, damping: np.ndarray) -> None:
        ...
        """
        assert len(damping) == len(
            self.joint_names
        ), "Damping array must match number of joints"
    ...
    def set_joint_friction(self, friction: np.ndarray) -> None:
        ...
        """
        assert len(friction) == len(
            self.joint_names
        ), "Friction array must match number of joints"
        for i, name in enumerate(self.joint_names):
